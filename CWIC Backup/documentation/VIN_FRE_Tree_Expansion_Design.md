...
- Treat the pending automatic-Geneva terminals, Amarante, day-`210` hold, Lorraine focus-order, and post-DBP southern-route engine checks as preflight. Record demonstrated regressions, but do not redesign an accepted parent lifecycle from preference alone.
...
- Complete the post-DBP closure preflight first: a DBP fall with the NLF already dead must announce Geneva on the next eligible daily check, and a funded NLF defeated later must produce one conclusion and the same automatic route once the peace gate passes.
- Complete the remaining response-package preflight: one human CEFEO Amarante run, the day-`210` hold and committed-loss terminals, Northwest/Lorraine in both focus orders, and the southern negotiation route.
