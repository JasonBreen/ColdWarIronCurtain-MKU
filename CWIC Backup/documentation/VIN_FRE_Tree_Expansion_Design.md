...
- Status: recorded by this checkpoint; targeted static verification passed. Campaign-front containment, repaired southern supply, MEO self-defense, Northwest balance, and the Hoa Binh lifecycle have engine acceptance. The Amarante payoff/clarity follow-up is implemented and awaits two human CEFEO engine checks; Lorraine and the post-DBP southern response still require dedicated engine checks.
...
- Once the Dien Bien Phu result is recorded, a surviving, non-capitulated NLF that still controls ground and remains at war triggers a VIN briefing and a temporary decision category. This is an explicit signifier that the northern set-piece did not end the southern war.
...
- Expanded Dien Bien Phu response packages, remaining FRE operations, and supporting narrative/AI content.
...
### Next content patch: Dien Bien Phu defensive responses

- Treat the pending Amarante, day-`210` hold, Lorraine focus-order, and post-DBP southern-route engine checks as preflight. Record demonstrated regressions, but do not redesign an accepted parent lifecycle from preference alone.
- Extend the existing Dien Bien Phu spine: `FRE_Operation_Castor` in `common/national_focus/FRE_50s_Indochina.txt`; `FRE_Events.23`, `.32`, `.33`, and `FRE_DBP.*` in `events/FRE_Events.txt`; `vin_dbp_record_outcome`, siege investment, and the daily siege clock in `common/scripted_effects/VIN_Campaign_Effects.txt`; and `USA_50s_Operation_Vulture` plus the `Operation_Vulture` flag in `common/national_focus/USA_FP_50s.txt`.
- Attach one defensive-response package to live VIN campaign ID `3`. The primary campaign remains the sole owner of objective classification, permanent result, territory, peace, Geneva state, and cleanup.
- Design bounded, mutually exclusive postures around a standard hold, maximum reinforcement, an Operation Condor breakout, and the existing Vulture diplomatic/air-support route. Each choice needs an up-front contract, a result-dependent payoff, AI handling, supersession/orphan handling, and common cleanup; no selection may grant its terminal reward immediately.
- Preserve the current siege-investment prerequisites, exact camp objective province `4529`, day-`150` clean threshold, day-`260` final deadline, and existing Dien Bien Phu/Geneva recorder.
- Add response diagnostics and scoped localization with the first implementation. Player-facing values must use historical names and plain outcomes instead of raw tags, state/province IDs, flag names, or resolver terminology.
...
- The Lorraine rear-depot objective uses the six provinces of state `881` (Dong Bac Bo) as the map's abstraction of the Clear River/Phu Doan rear area. Engine testing must confirm that a CEFEO or VIE controller is credited on the daily seven-day hold counter and that the targeted AI can reach the state without abandoning the Northwest defense.
- The post-DBP southern decisions deliberately stop short of direct VIN intervention. Funding escalates and sustains the NLF; negotiation pressure improves the existing Geneva path but does not force a peace with VIE.
...
- Complete the current response-package preflight: one human CEFEO Amarante run, the day-`210` hold and committed-loss terminals, Northwest/Lorraine in both focus orders, and both post-Dien Bien Phu southern-response routes.
- Audit the existing Castor, `FRE_DBP.*`, siege-clock, `USA_50s_Operation_Vulture`, and `Operation_Vulture` call sites before editing. Retain their working narrative and result hooks instead of starting a second operation path.
- Define one Dien Bien Phu defensive-response namespace and its parent-owned lifecycle contract for campaign ID `3`.
- Implement the standard-hold and maximum-reinforcement postures first, including cleanup, AI, outcome localization, and console diagnostics. Then connect Condor and Vulture through the same response package without bypassing the campaign or Geneva recorders.
